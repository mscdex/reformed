...
/*
  name: {
    ...
    required: true,
    rules: [
      { test: /^foo/, error: 'Wrong value format' }
    ]
  },
  avatar: {
    filename: '',
    rules: [
      { test: function(key, filename, filesize, cb) {
          cb(/\.jpg$/i.test(filename));
        },
  ...
  function onBBFile(key, stream) {
    if (cfg[key] && cfg[key].filename !== undefined) {
      ...
      if (typeof filename === 'string') {
        addValue = function() {
          var val = { filename: filename, size: filesize };
          if (multiple) {
            if (Array.isArray(data[key]))
              data[key].push(val);
            else if (data[key] !== undefined)
        ...
        addValue = function() {
          var val = { data: val, size: filesize };
          if (multiple) {
            if (Array.isArray(data[key]))
              data[key].push(val);
            else if (data[key] !== undefined)
